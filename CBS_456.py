...
from .CBS_456_dialog import CBS456Dialog
import os.path
import ogr
...
class CBS456:
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('CBS456', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/CBS_456/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u''),
            callback=self.run,
    ...
    def unload(self):
        ...
        QMessageBox.warning(self.dlg.show(), self.tr("CBS Warning"), self.tr(str(uyari)),QMessageBox.Ok)

    def shapefile_sec(self):
        try: 
            self.shapefilePath,x = QFileDialog.getOpenFileName(self.dlg, "Shapefile dosyasini secin", "" , "ESRI Shapefiles(*.shp *.SHP);; GeoJSON (*.GEOJSON *.geojson);; Geography Markup Language(*.GML)")
            # x dosyanin uzantisini veriyor ( ESRI Shapefiles(*.shp *.SHP))
